...
const AppContainer = () => {
    ...
    return (
        <div className="app-container">
            ...
            <SocialLinks socialLinks={socialLinks} onEditSocialLink={editSocialLink} />
            <button onClick={() => setVideoModalOpen(true)} className="add-video-button">
                Add Video
            </button>
            <VideoList videos={videos} onDelete={handleDeleteVideo} onReorder={reorderVideos} />
            {isVideoModalOpen && (
                <UploadModal
                    title="Upload Video Link"
                    fields={[{ name: "url", type: "text", placeholder: "Enter Video URL (x.com)", autoFocus: true }]}
                    onSave={addVideo}
            ...
            {isSocialLinkModalOpen && (
                <UploadModal
                    ...
                    fields={[{ name: "url", type: "text", placeholder: `Enter your ${editingPlatform} URL`, autoFocus: true }]}
                    initialData={{ url: socialLinks[editingPlatform] }}
                    onSave={(data) => updateSocialLink({ platform: editingPlatform, ...data })}
                    onClose={() => setSocialLinkModalOpen(false)}
                />
            )}
            {isErrorModalOpen && (
                <ErrorModal onClose={() => setErrorModalOpen(false)} message="You have reached the maximum limit of 10 videos. Please delete a video to upload a new one." />
            )}
        </div>
    );
};
